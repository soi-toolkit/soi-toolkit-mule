...
2. Configure SSL/TLS: configure keystore in an props-override file:
  soitoolkit-encryption-web-tool-config-override.properties  
  
  and put the file in dir $MULE_HOME/conf
  
  Example config:
	HTTPS_TLS_KEYSTORE=${mule.home}/conf/soitoolkit-encryption-tool-server.jks
	HTTPS_TLS_KEYSTORE_PASSWORD=password
	HTTPS_TLS_KEY_PASSWORD=password
	
	NOTE: find example props-override and keystore in src/environment.
